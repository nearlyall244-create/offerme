        ...
        <>
            <header>
                <nav
                    ...
                    className="group fixed z-20 w-full border-b border-dashed border-slate-200 bg-white/80 backdrop-blur md:relative dark:border-zinc-800 dark:bg-zinc-950/50 lg:dark:bg-transparent">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-6 py-3 lg:gap-0 lg:py-4">
                            <div className="flex w-full justify-between lg:w-auto">
                                ...
                                <button
                                    ...
                                    className="relative z-20 -m-2.5 -mr-4 block cursor-pointer p-2.5 lg:hidden text-slate-700 dark:text-slate-200">
                                    <Menu className="group-data-[state=active]:rotate-180 group-data-[state=active]:scale-0 group-data-[state=active]:opacity-0 m-auto size-6 duration-200" />
                                    <X className="group-data-[state=active]:rotate-0 group-data-[state=active]:scale-100 group-data-[state=active]:opacity-100 absolute inset-0 m-auto size-6 -rotate-180 scale-0 opacity-0 duration-200" />
                                </button>
                            </div>

                            <div className="bg-white dark:bg-zinc-900 group-data-[state=active]:block lg:group-data-[state=active]:flex mb-6 hidden w-full flex-wrap items-center justify-end space-y-8 rounded-3xl border border-slate-200 dark:border-zinc-800 p-6 shadow-2xl shadow-zinc-300/20 md:flex-nowrap lg:m-0 lg:flex lg:w-fit lg:gap-6 lg:space-y-0 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none dark:shadow-none dark:lg:bg-transparent">
                                    ...
                                    <ul className="space-y-6 text-base lg:flex lg:gap-8 lg:space-y-0 lg:text-sm">
                                        {menuItems.map((item, index) => (
                                            <li key={index}>
                                                <Link
                                                    to={item.href}
                                                    className="text-slate-600 hover:text-emerald-600 dark:text-zinc-400 dark:hover:text-emerald-400 block duration-150 font-medium">
                                                    <span>{item.name}</span>
                                                </Link>
                                            </li>
                                        ))}
                                ...
                                <div className="flex w-full flex-col space-y-3 sm:flex-row sm:gap-3 sm:space-y-0 md:w-fit lg:border-l lg:border-slate-200 dark:lg:border-zinc-800 lg:pl-6">
                                    <Button
                                        asChild
                                        variant="outline"
                                        size="sm">
                    ...
                    <div className="relative mx-auto max-w-5xl px-6 py-28 lg:py-20">
                        <div className="lg:flex lg:items-center lg:gap-12">
                            <div className="relative z-10 mx-auto max-w-xl text-center lg:ml-0 lg:w-1/2 lg:text-left">
                                <Link
                                    to="/shops"
                                    className="rounded-lg mx-auto flex w-fit items-center gap-2 border border-slate-200 dark:border-zinc-800 p-1 pr-3 lg:ml-0 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-colors">
                                    <span className="bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 font-semibold rounded-[calc(var(--radius,0.5rem)-0.25rem)] px-2 py-1 text-xs">New</span>
                                    <span className="text-sm font-medium text-slate-700 dark:text-slate-200">Hyperlocal OfferMe Directory</span>
                                    <span className="bg-slate-200 dark:bg-zinc-700 block h-4 w-px"></span>

                                    <ArrowRight className="size-4 text-emerald-600 dark:text-emerald-400" />
                                </Link>

                                <h1 className="mt-10 text-balance text-4xl font-bold md:text-5xl xl:text-5xl text-slate-900 dark:text-white tracking-tight">Production Ready Hyperlocal Discovery Blocks</h1>
                                <p className="mt-8 text-slate-600 dark:text-zinc-400 leading-relaxed">Discover nearby verified shops, local service providers, daily offers, and community events directly in your neighborhood with real-time updates.</p>

                                <div>
                                    <form
                                        onSubmit={(e) => e.preventDefault()}
                                        className="mx-auto my-10 max-w-sm lg:my-12 lg:ml-0 lg:mr-auto">
                                        <div className="bg-white dark:bg-zinc-900 has-[input:focus]:ring-emerald-500/20 relative grid grid-cols-[1fr_auto] items-center rounded-[1rem] border border-slate-200 dark:border-zinc-800 pr-1 shadow-sm shadow-zinc-950/5 has-[input:focus]:ring-2">
                                            <Mail className="text-slate-400 pointer-events-none absolute inset-y-0 left-5 my-auto size-5" />

                                            <input
                                                placeholder="Your mail address"
                                                className="h-14 w-full bg-transparent pl-12 focus:outline-none text-slate-900 dark:text-white placeholder:text-slate-400 text-sm"
                                                type="email"
                                            />
                                    ...
                                    <ul className="list-inside list-disc space-y-2 text-slate-600 dark:text-zinc-400 text-sm">
                                        <li>Faster Local Discovery</li>
                                        <li>Modern Verified Listings</li>
                                        <li>100% Hyperlocal & Community Focused</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div className="absolute inset-0 -mx-4 rounded-3xl p-3 lg:col-span-3">
                            <div aria-hidden className="absolute z-[1] inset-0 bg-gradient-to-r from-slate-50 dark:from-zinc-950 from-35% pointer-events-none" />
                            <div className="relative">
                                <img
                                    className="hidden dark:block rounded-2xl object-cover border border-zinc-800 shadow-2xl"
                                    src="https://images.unsplash.com/photo-1556742049-0a670fc8078a?auto=format&fit=crop&q=80&w=1920"
                                    alt="app illustration dark"
                                    width={2796}
                                    height={2008}
                                />
                                <img
                                    className="dark:hidden rounded-2xl object-cover border border-slate-200 shadow-2xl"
                                    src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=1920"
                                    alt="app illustration light"
                                    width={2796}
                                    height={2008}
                                />
                            </div>
...
const Logo = ({ className }) => {
    return (
        <svg
            ...
            className={cn('h-5 w-auto text-emerald-600 dark:text-emerald-400', className)}>
            <path
